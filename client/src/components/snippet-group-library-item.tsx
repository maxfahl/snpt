import React, { FunctionComponent, MouseEvent } from "react";
import EditableTextButton from "./editable-text-button";
import { SnippetGroup } from "../models/snippet-group";
import { useOvermind } from "../overmind";

type SnippetGroupLibraryItemProps = {
    snippetGroup: SnippetGroup;
};

const SnippetGroupLibraryItem: FunctionComponent<SnippetGroupLibraryItemProps> = ({
    snippetGroup,
}) => {
    const {
        state: { selectedSnippetGroup, selectedSnippet },
        actions: { setSelectedSnippetGroup, setSelectedSnippet },
    } = useOvermind();

    const onSelectGroup = (e: MouseEvent, model: SnippetGroup) => {
        setSelectedSnippetGroup(snippetGroup.id);
    };

    return (
        <div className="pt-4 px-4">
            <EditableTextButton
                model={snippetGroup}
                isSelected={snippetGroup.id === selectedSnippetGroup}
                onSelect={onSelectGroup}
            />
        </div>
    );
};

/***
 <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
 <path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd" />
 </svg>
 */

export default SnippetGroupLibraryItem;
